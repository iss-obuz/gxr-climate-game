...
def headset_name(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(item in df.columns for item in headset_names):
        player_to_index = {
            value: key for key, value in df["PlayerIndexToPlayerNr"][0].items()
        }
        index_to_nick = {
            value: key for key, value in df["NickNameToPlayerIndex"][0].items()
        }
        player_to_nick = {
            str(key): index_to_nick[int(value)]
            for key, value in player_to_index.items()
        }
